...
Set_config_user(){
	...
	read -e -p "(默认: cloudcave):" ssr_user
	[[ -z "${ssr_user}" ]] && ssr_user="cloudcave"
...
Set_config_password(){
	echo "请输入要设置的用户 密码"
	read -e -p "(默认: cloudcave):" ssr_password
	[[ -z "${ssr_password}" ]] && ssr_password="cloudcave"
